...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" wire:model="{{ $prefix }}.shelf"
                class="w-full rounded-lg p-2 bg-gray-100 border-blue-400">
            @error($prefix . '.shelf')
        ...
        <div>
            ...
            <input type="number" wire:model="{{ $prefix }}.amount" min="0"
                class="w-full rounded-lg bg-gray-100 p-2"
                onwheel="
        event.preventDefault();
        this.stepUp(event.deltaY < 0 ? 1 : -1);
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" wire:model="{{ $prefix }}.manufacturer"
                class="w-full rounded-lg p-2 bg-gray-100">
            @error($prefix . '.manufacturer')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        <div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div x-data="{ customRam: $wire.{{ $prefix }}.ram !== null && ![8, 16, 32].includes($wire.{{ $prefix }}.ram) }">
            ...
            <div class="flex w-full gap-2 flex-wrap">
                ...
                <button type="button" @click="customRam = true" class="rounded-lg px-4 py-2"
                    :class="customRam ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200'">
                    Anderes
                </button>

                <input x-show="customRam" type="number" wire:model="{{ $prefix }}.ram" min="0"
                    class="mt-2 w-full rounded-lg p-2 bg-gray-100">
            </div>
        ...
        <div x-data="{ customSsd: $wire.{{ $prefix }}.ssd !== null && ![128, 256, 512].includes($wire.{{ $prefix }}.ssd) }">
            ...
            <div class="flex w-full gap-2 flex-wrap">
                ...
                <button type="button" @click="customSsd = true" class="rounded-lg px-4 py-2"
                    :class="customSsd ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200'">
                    Anderes
                </button>

                <input x-show="customSsd" type="number" wire:model="{{ $prefix }}.ssd" min="0"
                    class="mt-2 w-full rounded-lg p-2 bg-gray-100">
            </div>
    ...
    <div>
        ...
        <input type="number" wire:model="{{ $prefix }}.price" min="0"
            class="w-full rounded-lg p-2 bg-gray-100">
        @error($prefix . '.price')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
